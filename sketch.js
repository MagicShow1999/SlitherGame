...
function draw() {
  ...
  if (snake.length >= 150 && snake.length < 250) {
    scaleVal = 0.8
  }
  if (snake.length >= 250 && snake.length < 500) {
    scaleVal = 0.5
  ...
  fill(255)
  text(state,snake.x - 100,snake.y)
  // console.log(snake.x, snake.y)
  // console.log(mouseX, mouseY)
  }
  pop();
  textSize(15 / scaleVal )
  fill(255)
  text("Your length: " + snake.length, 20 /scaleVal,20 / scaleVal)
  text("Rank:", 20, 40 / scaleVal)
  let counter = 60 / scaleVal
  // sort snake objects by their length
  totalSnakes.sort(function(a, b) {
    var keyA = a.length
    var keyB = b.length
    // Compare the 2 dates
  ...
  // write them on the top-left screen
  totalSnakes.forEach( s => {
    if (s.state == "LIVE") {
      text(s.name + " : " + s.length, 20, counter)  
      counter += 20 / scaleVal
    }
  ...
  for (let i = 0; i < nutrientsArr.length; i++) {
    if(nutrientsArr[i] && nutrientsArr[i].display(snake.x, snake.y, snake.size)) {
        // if (snake.name == 'Player') {
          // console.log("eaten by", snake.name)
          // console.log(i)
        // }
...
function AIheadDegree(x, y, nutrientX, nutrientY, fastMode) {
